...
func newTestAPI(t *testing.T) *httptest.Server {
	...
	ns, err := store.Open(filepath.Join(t.TempDir(), "notes.db"))
	if err != nil {
		t.Fatalf("store.Open: %v", err)
	}
	t.Cleanup(func() { ns.Close() })

	return httptest.NewServer(New(ns).Handler())
}
...
func TestNotesSurviveServerRestart(t *testing.T) {
	...
	ts := httptest.NewServer(New(ns).Handler())
	resp, body := doReq(t, ts, http.MethodPost, "/notes",
	...
	ts2 := httptest.NewServer(New(reopened).Handler())
	defer ts2.Close()

	resp, body = doReq(t, ts2, http.MethodGet, "/notes/1", "", false)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("get after restart = %d, want 200 (body: %s)", resp.StatusCode, body)
	}
	if !strings.Contains(string(body), `"title":"persistent"`) {
		t.Errorf("note lost after restart: %s", body)
	}
}
